...
// TODO: NOTE: Pack has an embedded sequence implementation
...
// TODO: shifts and BitsAtEdge only work for unsigned!
// Returns true if all set bits are contiguous and include the LSB and/or MSB
template <class T>
constexpr bool BitsAtEdge(T value) {
  return (
      ((value & (value+1)) == 0) ||
      (static_cast<T>(~value) & static_cast<T>(~value+1)) == 0);
}
...
NX_FORCEINLINE constexpr EnableIf<Bool<
    ...
    T> Shift(T value) {
  return static_cast<T>(value << -kOffset);
}
template <ssize_t kOffset, class T>
NX_FORCEINLINE constexpr EnableIf<Bool<
    (kOffset > 0 && kOffset < static_cast<ssize_t>(Bits<T>::Size()))>,
...
template <ssize_t kOffset, class Mask, class T>
NX_FORCEINLINE constexpr EnableIf<Not<Bool<(
    BitsAtEdge(Mask::value) &&
    BitsAtEdge(Shift<kOffset>(Mask::value)) && kOffset)>>,
    T> ShiftMask(T value) {
  return Shift<kOffset>(value & Mask::value);
}
// if the mask touches the edge before shifting, and after shifting, no need to
// and it.  TODO: check signed
template <ssize_t kOffset, class Mask, class T>
NX_FORCEINLINE constexpr EnableIf<Bool<(
    BitsAtEdge(Mask::value) &&
    BitsAtEdge(Shift<kOffset>(Mask::value)) && kOffset)>,
...
NX_FORCEINLINE constexpr EnableIf<Bool<(
  ...
  // NOTE: if the mask touches one side and moves to the other, we don't need
  // to & the value with the mask.  ShiftMask handles this.
  return ShiftMask<Traits::shiftDelta, fullMask>(value) | toField<
...
class SandboxApplication : public nx::Application {
 public:
  int main() {
    ...
    unsigned int value =
        static_cast<unsigned int>(0b00110011 * arguments().size());
    ...
    asm("# im in ur loop" : : );
    std::cout << value << " = " << resultA << ", " << resultB << ", " << resultC << std::endl;

    asm("# im in ur loop" : : );
    std::cout << +toField<uint8_t, 4, 5, 6, 7>(0b1011) << std::endl;
